...
use kite::{Args, ProgressBar, Logger, Level, Color, Style};
use std::thread;
...
fn main() {
    ...
    if args.has_flag("version") {
        println!("{}", Color::Cyan.paint("Kite Example CLI v0.1.0").style(Style::Bold));
        return;
    }
    
    // Get configuration from arguments
    let iterations = args.get_option("iterations")
    ...
    logger.info(&format!("{}", Color::Green.paint("Starting application...").style(Style::Bold)));
    ...
    // Final output
    logger.info(&format!("Results saved to: {}", Color::Cyan.paint(output_file)));
    logger.info(&format!("{}", Color::Green.paint("✓ Done!").style(Style::Bold)));
...
fn show_help() {
    println!("{}", Color::Cyan.paint("Kite Example CLI").style(Style::Bold));
    println!();
    println!("USAGE:");
    ...
    println!("    {} --verbose -o results.txt", std::env::args().next().unwrap());
}
